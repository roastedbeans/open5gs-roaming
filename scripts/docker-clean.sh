...
CONTAINERS=$(docker ps -a | grep -E 'open5gs|sepp|webui|smf|udm|amf|udr|upf|pcf|nrf|ausf|nssf|bsf' | awk '{print $1}')

if [ -n "$CONTAINERS" ]; then
  echo -e "${BLUE}Found the following containers to remove:${NC}"
  docker ps -a | grep -E 'open5gs|sepp|webui|smf|udm|amf|udr|upf|pcf|nrf|ausf|nssf|bsf'
...
IMAGES=$(docker images | grep -E 'open5gs|sepp|webui|smf|udm|amf|udr|upf|pcf|nrf|ausf|nssf|bsf' | awk '{print $1":"$2}')

if [ -n "$IMAGES" ]; then
  echo -e "${BLUE}Found the following images to remove:${NC}"
  docker images | grep -E 'open5gs|sepp|webui|smf|udm|amf|udr|upf|pcf|nrf|ausf|nssf|bsf'
  docker rmi -f $IMAGES
