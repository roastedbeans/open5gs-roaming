...
class AutoencoderPreprocessor:
    """Preprocessing pipeline for Autoencoder model"""
    ...
    def preprocess_for_training(self, data_path, batch_size=64, val_split=0.125, test_split=0.20):
        ...
        # Extract features and labels
        X = df.drop(columns=['label']).values.astype(np.float32)  # (N, 76)
        y_str = df['label'].values

        # Encode labels
        y = self.label_encoder.fit_transform(y_str).astype(np.int64)
        ...
        return {
            ...
            'input_shape': (76,)  # 76 features
        }

    def preprocess_for_inference(self, data):
        """Preprocess data for Autoencoder inference

        Args:
            data: Input data (numpy array, pandas DataFrame, or dict)

        Returns:
            torch.Tensor: Preprocessed tensor ready for Autoencoder input (batch_size, 76)
        """
        ...
        # Convert to tensor (Autoencoder expects 2D: batch_size, features)
...
def load_autoencoder_preprocessor(scaler_path=None, encoder_path=None):
    """Load a fitted Autoencoder preprocessor
    ...
    Returns:
        AutoencoderPreprocessor: Fitted preprocessor
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Processed shape: {processed.shape}")  # Should be (1, 76)
    except Exception as e:
        print(f"Error: {e}")
