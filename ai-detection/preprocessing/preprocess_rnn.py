...
class RNNPreprocessor:
    """Preprocessing pipeline for Vanilla RNN models"""
    ...
    def preprocess_for_training(self, data_path, batch_size=64, val_split=0.125, test_split=0.20):
        """Preprocess data for RNN/LSTM/GRU training
        ...
        # Reshape for RNN: (batch, seq_len=76, input_size=1)
        def to_seq(x):
            return torch.tensor(x, dtype=torch.float32).unsqueeze(-1)  # (N, 76, 1)
        ...
        return {
            ...
            'input_shape': (76, 1)  # (seq_len, input_size)
        }

    def preprocess_for_inference(self, data):
        """Preprocess data for RNN/LSTM/GRU inference

        Args:
            data: Input data (numpy array, pandas DataFrame, or dict)

        Returns:
            torch.Tensor: Preprocessed tensor ready for RNN input (batch_size, 76, 1)
        """
        ...
        # Convert to tensor and reshape for RNN: (batch_size, seq_len, input_size)
...
def load_rnn_preprocessor(scaler_path=None, encoder_path=None):
    """Load a fitted RNN preprocessor
    ...
    Returns:
        RNNPreprocessor: Fitted preprocessor
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Processed shape: {processed.shape}")  # Should be (1, 76, 1)
    except Exception as e:
        print(f"Error: {e}")
