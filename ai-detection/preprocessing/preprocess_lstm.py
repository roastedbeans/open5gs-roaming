...
class LSTMPreprocessor:
    ...
    def preprocess_for_training(self, data_path, batch_size=64, val_split=0.125, test_split=0.20):
        ...
        # Reshape for LSTM: (batch, seq_len=76, input_size=1)
        def to_seq(x):
    ...
    def preprocess_for_inference(self, data):
        """Preprocess data for LSTM inference

        Args:
            data: Input data (numpy array, pandas DataFrame, or dict)

        Returns:
            torch.Tensor: Preprocessed tensor ready for LSTM input (batch_size, 76, 1)
        """
        ...
        # Convert to tensor and reshape for LSTM: (batch_size, seq_len, input_size)
...
def load_lstm_preprocessor(scaler_path=None, encoder_path=None):
    """Load a fitted LSTM preprocessor
    ...
    Returns:
        LSTMPreprocessor: Fitted preprocessor
    """
