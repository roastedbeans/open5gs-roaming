...
class MLPPreprocessor:
    """Preprocessing pipeline for MLP model"""
    ...
    def preprocess_for_training(self, data_path, batch_size=64, val_split=0.125, test_split=0.20):
        ...
        # Convert to PyTorch tensors (MLP expects 2D: batch_size, features)
    ...
    def preprocess_for_inference(self, data):
        """Preprocess data for MLP inference

        Args:
            data: Input data (numpy array, pandas DataFrame, or dict)

        Returns:
            torch.Tensor: Preprocessed tensor ready for MLP input (batch_size, 76)
        """
        ...
        # Convert to tensor (MLP expects 2D: batch_size, features)
...
def load_mlp_preprocessor(scaler_path=None, encoder_path=None):
    """Load a fitted MLP preprocessor
    ...
    Returns:
        MLPPreprocessor: Fitted preprocessor
    """
